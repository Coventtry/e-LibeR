<?php
require 'conectatew.php'; // Conexión PDO centralizada
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Datos del formulario - CORREGIDO LOS NOMBRES DE VARIABLES
    $id = $_POST['id'];
    $titulo = trim($_POST['titulo']);
    $autor = trim($_POST['autor']);
    $anio_publicacion = trim($_POST['anio_publicacion']); // Con tilde
    $disponibilidad = trim($_POST['disponibilidad']); // Exactamente como en el formulario
    ...
    try {
        ...
        // 1. Actualizar datos básicos del material - VARIABLES CORREGIDAS
        ...
        $stmt_material->execute([
            ':titulo' => $titulo,
            ':autor' => $autor,
            ':anio_publicacion' => $anio_publicacion, // Con tilde
            ':disponibilidad' => $disponibilidad, // Exacto
            ':area_id' => $area_id,
            ':categoria' => $categoria,
            ':editorial' => $editorial,
            ':id' => $id
        ...
        // Primero verificamos si ya existe una ubicación para este material
        ...
        if ($existe_ubicacion) {
            ...
            $sql_ubicacion = "INSERT INTO ubicaciones_fisicas 
                            (material_id, pasillo, estante, nivel, codigo_ubicacion)
                            VALUES (:material_id, :pasillo, :estante, :nivel, :codigo_ubicacion)";
        }
...
<body>
    <div class="container">
                        ...
                        <select class="form-control" name="area_id" required>
                            <?php foreach ($areas as $area): ?>
                                <option value="<?= $area['id'] ?>" 
                                    <?= $area['id'] == $material_seleccionado['area_id'] ? 'selected' : '' ?>
                                    data-abreviado="<?= $area['Abreviado'] ?>">
                ...
                <div class="card mt-3 mb-4">
                    ...
                    <div class="card-body">
                        <div class="location-grid">
                            <div class="form-group">
                                ...
                                <select class="form-control" name="pasillo" required>
                                    ...
                                    <?php foreach (range('A', 'J') as $letra): ?>
                                        <option value="<?= $letra ?>" <?= ($material_seleccionado['pasillo'] ?? '') == $letra ? 'selected' : '' ?>>
                                            <?= $letra ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="estante" required>
                                    ...
                                    <?php foreach (range(1, 30) as $numero): ?>
                                        <option value="<?= $numero ?>" <?= ($material_seleccionado['estante'] ?? '') == $numero ? 'selected' : '' ?>>
                                            <?= $numero ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            ...
                            <div class="form-group">
                                <label for="nivel">Nivel (1-4)</label>
                                <select class="form-control" name="nivel" required>
                                    <option value="">Seleccione nivel</option>
                                    <?php foreach (range(1, 4) as $nivel): ?>
                                        <option value="<?= $nivel ?>" <?= ($material_seleccionado['nivel'] ?? '') == $nivel ? 'selected' : '' ?>>
                                            <?= $nivel ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                        ...
                        <div class="form-group mt-3">
                            ...
                            <input type="text" class="form-control" id="clasificacion-fisica" 
                                   value="<?= $material_seleccionado['clasificacion_fisica'] ?? '' ?>" readonly>
                        </div>
                    </div>
                </div>
        ...
        <a href="ingreso_bibliotecario.php" class="volver d-block text-center mt-4">← Volver al menú principal</a>
    </div>

    <footer class="text-center">
        <p>Practica Profesionalizante I<br>
        Sistema de Gestión Bibliotecaria</p>
    </footer>
